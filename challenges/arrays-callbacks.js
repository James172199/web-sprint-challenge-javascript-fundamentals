...
zooAnimals.forEach(function (entry) {
  displayNames[x] =
    "Name: " + entry.animal_name + " Scientific: " + entry.scientific_name;
  x++;
});
...
x = 0;
zooAnimals.forEach(function (entry) {
  lowCaseAnimalNames[x] = entry.animal_name;
  x++;
});
lowCaseAnimalNames = lowCaseAnimalNames.map(function (a) {
...
*/
let lowPopulationAnimals = zooAnimals.filter((animal) => {
...
*/
let populationTotal = zooAnimals.reduce(function (accumulator, zooAnimals) {
...
console.log(greeting("James", "Stimpson"));
